...
def describe_video():
  ...
  prompt = "You are a helpfull assistant. Summarize the video based on the following frame filenames:\n" + "\n".join(descriptions)
...
def turn_into_story(summary):
  prompt = f"Turn the following video summary inta a narrative story with characters, setting, conflict, and resolution:\n\n{summary}"
  return model.invoke(prompt)

st.title("VasuAI - YouTube/Uploaded Video Summarizer Using Groq LLM")
st.image("VasuAI_Transperent.jpg") 
...
if "summary" in st.session_state:
  ...
  with col1:
    if st.button(" Rewrite Summary Nicely"):
      with st.spinner("Rewrite summary..."):
